...
setClass("ChangeDetection",
         ...
         prototype(name = NA_character_,
                   ...
                   r_command = '/Users/lisahopcroft/opt/anaconda3/bin/Rscript',
                   reverse = FALSE,
...
run_r_script = function(cd, i, script_name, input_name, output_name, module_folder, ... ) {
    ...
    system( cmd, wait=TRUE, ignore.stdout=TRUE, ignore.stder=TRUE )
}
...
option_list = list(
    ...
    make_option(c("-p", "--pvalue"), type="numeric", default=default_values@p_threshold,
                help="the p value threshold to use when identifying breaks [default=%default]", metavar="numeric"),
    make_option(c("-r", "--ratio"), type="numeric", default=default_values@ratio_threshold,
                help="the ratio threshold to use when identifying breaks [default=%default]", metavar="numeric"),
